...
def calculateH(r, t, fp, fpp): 
    ...
    if r <= 0.8:                       
        X = 2 / ((6 * (r - t)) ** (1/3))
        Y = 1.6509636244473134*(r - t)**(2/3) 
        dX_dr = -0.3668808054 / ((r - t) ** (4/3))
        dX_dt = -dX_dr
        dY_dr = 1.100642416298209 / ((r - t)**(1/3))
        dY_dt = -dY_dr 
    ...
    #Glue Region is for the gluing of the metric, and is different than gluing for f in fGenerator
        ...
        dX_dr = ((0.3147944506877429 + 221.64186658205165*r**10 - 27.28675990405975*r**11 + r**9*(-768.8853224449925 - 28.066381615604318*t) + 0.573283980986305*t + 
                ...
                r**3*(53.00757425014704 + 47.767873798856755*t + 8.477385842364002*t**2))/((0.15247058823529414 + 0.8470588235294118*r - 1.7294117647058822*r**2 + r**3)**2*((0.648 + 3.6*r - 7.35*r**2 + 4.25*r**3)**2*(-0.8 + 3*r - 1.25*r**2 - t))**0.3333333333333333*(0.64 - 2.4*r + r**2 + 0.8*t)))
        dX_dt = ((-0.032915043135895446 + 12.877516270689048*r**9 - 1.55924342308913*r**10 + 
                ...
                r**5*(45.694958513673356 + 33.69090048686677*t))/((0.15247058823529414 + 0.8470588235294118*r - 1.7294117647058822*r**2 + r**3)**2*((0.648 + 3.6*r - 7.35*r**2 + 4.25*r**3)**2*(-0.8 + 3*r - 1.25*r**2 - t))**0.3333333333333333*(0.64 - 2.4*r + r**2 + 0.8*t)))
        ...
        dY_dt = (5.847162836584234*(0.15247058823529414 + 0.8470588235294118*r - 1.7294117647058822*r**2 + r**3)*(0.64 - 2.4*r + r**2 + 0.8*t))/((0.648 + 3.6*r - 7.35*r**2 + 4.25*r**3)*(0.8 - 3*r + 1.25*r**2 + t)**2)**0.6666666666666666
...
def main():    
    fileni = "test5.txt"
    data = fGenerator.parametricGenerator(fileni)
